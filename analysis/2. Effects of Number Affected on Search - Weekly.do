...
cap use "Google Data\final_weekly_search_data.dta"
...
egen num_weeks_on_from0 = rowtotal(num_weeks_on1-num_weeks_on4)
...
save Texas_UI_Data/readyforweeklyregs.dta, replace

stop

***Main Regressions
areg ljob_search all_extensions_1week yy* mm*, ab(year_month)
areg ljob_search all_extensions_1week holiday unemp_rate msas*, ab(year_month)
areg ljob_search all_extensions_2week holiday unemp_rate msas*, ab(year_month)
areg ljob_search all_extensions_2week holiday unemp_rate msas* [aweight = labor_force], ab(year_month)

areg ljob_search all_extensions_1week lunemp_X_announce_1week lunemp_rate holiday msas* [aweight = labor_force], ab(year_month)
areg ljob_search all_extensions_2week lunemp_X_announce_2week lunemp_rate holiday msas* [aweight = labor_force], ab(year_month)
areg ljob_search all_extensions_4week lunemp_X_announce_4week lunemp_rate holiday msas* [aweight = labor_force], ab(year_month)

areg ljob_search all_extensions_2week *weeks_2week unemp_rate holiday msas* [aweight = labor_force], ab(year_month)

***
areg ljob_search all_extensions_1week all_extensions_2week all_extensions_4week unemp_rate holiday msas* [aweight = labor_force], ab(year_month)

***Include all fractional composition of UI claimants
areg ljob_search all_extensions_2week *weeks_2week num_on_ui* unemp_rate holiday msas* [aweight = labor_force], ab(year_month)

***Individual Extension Regressions
areg ljob_search tier2_2week tier3_2week tier4_2week tier1_13_2week tier1_20_2week eb13_2week eb20_2week unemp_rate holiday msas*, ab(year_month)


***Scott regs
*here note total_on_ui is negative and unemp_rate is positive; captures that people on UI search less
areg ljob_search all_extensions_2week total_on_ui num_ui_X_announce_2week unemp_rate holiday msas* [aweight = labor_force], ab(year_month) cluster(msa)
areg ljob_search all_extensions_2week total_on_ui_labor num_ui_X_announce_2week unemp_rate holiday msas* [aweight = labor_force], ab(year_month) cluster(msa)

*Don't see anything systematic when including lots of bins
areg ljob_search num_ui_4weeks-num_ui_75weeks unemp_rate holiday msas* [aweight = labor_force], ab(year_month) cluster(msa)
areg ljob_search num_ui_0_10_weeks-num_ui_20_30_weeks unemp_rate holiday msas* [aweight = labor_force] if (year<2008 | (year==2008 & week<28)), ab(year_month) cluster(msa)

*here we don't see spike around expiration, only around initial payments (pre EUC regimes)
areg ljob_search num_ui_4weeks-num_ui_24weeks unemp_rate holiday msas* [aweight = labor_force]  if (year<2008 | (year==2008 & week<28)), ab(year_month) cluster(msa)
areg ljob_search num_ui_4weeks-num_ui_39weeks unemp_rate holiday msas* [aweight = labor_force]  if (year==2008 & week>29 & week<48), ab(year_month) cluster(msa)

**Just focusing on weeks around extensions
areg ljob_search total_on_ui unemp_rate holiday msas* [aweight = labor_force] if number_on_ui !=.|l.number_on_ui!=.|l2.number_on_ui!=.|f.number_on_ui!=. |f2.number_on_ui!=., ab(date) cluster(date)
...
areg ljob_search total_on_ui unemp_rate holiday msas* [aweight = population] if number_on_ui !=.|l.number_on_ui!=.|l2.number_on_ui!=.|f.number_on_ui!=. |f2.number_on_ui!=., ab(year_month) cluster(msa_year)
